...
| 16 | Cover page: logo left of text, single title, no footer | DONE — template + pdf.py updated |
| 17 | No horizontal bars under # and ## headings | DONE — removed titleformat after-code |
| 18 | Hotel: parking bullet list + photo in PDF | DONE — format_hotel() updated |
| 19 | Restaurant labels italic style (*Address:* etc.) | DONE — restaurants.py format updated |
| 20 | Route: charging stations/summary on new line | DONE — charging.py format updated |
| 21 | Restaurant map in Planning section (PNG) | DONE — selenium screenshot embedded in PDF |
| 22 | Weekend_Guides dir + city-based filename | DONE — config + pdf.py + download endpoint |
...
### PDF formatting — all Comments.txt issues addressed

Changes made in response to the reference PDF (WeekendGuide_Aachen) + Comments.txt:

**Cover page fixes:**
- Logo (seahorse SVG) now positioned left of "DBG TRAVEL" text at the same height on cover page
- Merged "Weekend Guide Aachen Germany" + "Weekend Travel Guide" into single title: "Aachen Weekend Travel Guide"
- Removed "Generated by DBG Travel" footer from cover page
- Set LaTeX titleformat `\section` and `\subsection` to have no trailing horizontal rules (bars removed)

**Hotel section:**
- Parking garages now render as proper markdown bullet list with bold names (`- **Name** — addr — [Google Maps]`)
- "Nearby indoor parking garages:" heading is bold
- Hotel photo from Google Places API embedded as markdown image in the formatted text (shows in both web UI and PDF)
- Removed standalone `<img>` element from frontend Hotel tab — photo now renders through ReactMarkdown instead

**Restaurant section:**
- Label fields now use italic style matching hotel section: `*Address:*`, `*Walk:*`, `*Rating:*`, `*Website:*`, `*Google Maps:*`

**Route section:**
- "Charging stations used:" header followed by blank line, then station entries as plain bold text (no dash prefix)
- "Trip summary:" header followed by blank line, then data on its own line
- Both sections have proper blank-line separation for correct markdown paragraph rendering

**Planning section:**
- Restaurant map (Folium HTML with cuisine-colored markers) converted to PNG via headless Firefox+Selenium
- Injected as `## Restaurant Map` subsection at end of Planner — numbered 6.4 in the PDF TOC
- Map referenced via relative path in markdown so pandoc resolves it correctly

**Naming and saving:**
- PDFs now saved to `Weekend_Guides/` directory (config `WEEKEND_GUIDES_DIR`)
- Filename format: `weekend_guide_{city}_{country}_{timestamp}.pdf`
- Preview iframe already exists in the Brochure tab (600px)
- Download served via `/api/pdf/download/` endpoint (checks Weekend_Guides first, then brochures/)

**New dependencies:**
- `selenium` added to backend venv for headless Firefox map-to-PNG conversion
- Requires geckodriver (`/snap/bin/geckodriver` on this system)

All changes verified: backend imports OK, frontend build OK, lint clean.
